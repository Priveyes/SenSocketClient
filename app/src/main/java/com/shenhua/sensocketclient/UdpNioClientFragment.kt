...
import com.shenhua.libs.sensocketudp.nio.UdpNioClient
 ...
 */
class UdpNioClientFragment : BaseFragment() {

    private var udpNioClient: UdpNioClient? = null

    override fun connect() {
        executor!!.execute {
            val socketAddress = InetSocketAddress(etHost.text.toString(), etPort.text.toString().toInt())
            if (udpNioClient == null) {
                udpNioClient = UdpNioClient(mMessageProcessor, mConnectResultListener)
                udpNioClient!!.setConnectAddress(arrayOf(UdpAddress(socketAddress.hostName, socketAddress.port)))
            }
            udpNioClient!!.connect()
        }
    }

    override fun send() {
        executor!!.execute {
            mMessageProcessor.send(udpNioClient!!, etSend.text.toString().toByteArray())
        }
    }

    override fun disConnect() {
        udpNioClient?.disconnect()
    }
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        activity!!.title = getString(R.string.app_name) + "-Udp Nio"
    }
